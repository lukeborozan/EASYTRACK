            ...
            if args.save_id_crops:
                for d in r.boxes:
                    ...
                    save_one_box(
                        ...
                        r.orig_img.copy(),
                        file=(
                            yolo.predictor.save_dir / 'mot' /
                            str(int(d.cls.cpu().numpy().item())) /
                            str(int(d.id.cpu().numpy().item())) / f'{frame_idx}.jpg'
                        ),
                        BGR=True
                    )
    #if args.save_mot == True:
...
#def process_video2(video_path, output_dir):
    #opt = parse_opt()
...
#def process_video(video_path, output_dir, model_path):
    #opt = parse_opt()
    #opt.source = video_path #set source to video_path
    #opt.project = output_dir #set output
    #opt.yolo_model = model_path
    #run(opt)
