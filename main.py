...
def ball_is_stationary(ball_positions, thresholddistance, min_duration):
    frames_stationary = 0
    realframenumber = 0
    prev_position = None
    for frame_number, position in ball_positions:
        if prev_position is not None:
            distance_moved = distance(prev_position, position)
            if distance_moved < thresholddistance:
                frames_stationary += 1
                if frames_stationary >= min_duration:
                    realframenumber = frame_number
            else:
                frames_stationary = 0
...
if getstats:
    path_txt = path + '.txt'
    ball_positions = parse_center_ball(path_txt, ball_class=2)
    st.write(ball_positions)

    stationary_frame = ball_is_stationary(ball_positions, thresholddistance = 100, min_duration = 1)
    if stationary_frame:
